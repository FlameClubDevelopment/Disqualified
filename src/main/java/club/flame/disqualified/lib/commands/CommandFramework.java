        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(DisqualifiedLib.INSTANCE.getNoPermissionMessage()); // TODO change
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(DisqualifiedLib.INSTANCE.getOnlyGameCommandMessage());
                    return true;
                }
...
//            }
        ...
        if (DisqualifiedLib.INSTANCE.isExcludeCommands()) {
            config.set("EXCLUDE-COMMANDS", DisqualifiedLib.INSTANCE.getExcludeCommandsList());
        }
        config.set("TOTAL-REGISTER-COMMANDS", config.getKeys(false).size());
        try {
            config.save(file.getFile());
        } catch (IOException e) {
            DisqualifiedLib.INSTANCE.getPlugin().getLogger().warning("[Commands] Error in register commands.yml");
        }
    }
    ...
    public void registerCommands(Object obj, List<String> aliases) {
        for (Method m : obj.getClass().getMethods()) {
            if (m.getAnnotation(Command.class) != null) {
                ...
                boolean parse = false;
                if (DisqualifiedLib.INSTANCE.isExcludeCommands()) {
                    if (DisqualifiedLib.INSTANCE.getExcludeCommandFile() != null && DisqualifiedLib.INSTANCE.getExcludeCommandsPath() != null) {
                        for (String disabled : DisqualifiedLib.INSTANCE.getExcludeCommandsList()) {
                ...
                if (parse) {
                    Bukkit.getConsoleSender().sendMessage(DisqualifiedLib.INSTANCE.getDisableCommandMessage().replace("<command>", command.name()));
                    continue;
                }
