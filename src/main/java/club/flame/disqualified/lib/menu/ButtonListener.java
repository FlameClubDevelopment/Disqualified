package club.flame.disqualified.lib.menu;
...
public class ButtonListener implements Listener {
		...
		if (openMenu != null) {
			...
			if (openMenu.getButtons().containsKey(event.getSlot())) {
				...
				if (event.isCancelled()) {
					Bukkit.getScheduler().runTaskLater(DisqualifiedLib.INSTANCE.getPlugin(), () -> player.updateInventory(), 1L);
				}
			} else {
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		player.setMetadata("scanglitch", new FixedMetadataValue(DisqualifiedLib.INSTANCE.getPlugin(), true));
	}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (player.hasMetadata("scanglitch")) {
			player.removeMetadata("scanglitch", DisqualifiedLib.INSTANCE.getPlugin());
