...
public class TipsRunnable extends BukkitRunnable {
    ...
    List<String> messageTips = Disqualified.getInstance().getSettingsConfig().getConfiguration().getStringList("SETTINGS.TIPS.TIPS-LIST");
    AtomicInteger normalTip = new AtomicInteger(0);

    @Override
    public void run() {
        String tipsMode = Disqualified.getInstance().getSettingsConfig().getConfiguration().getString("SETTINGS.TIPS.MODE");
        switch (tipsMode) {
            case "normal":
